...
struct mlp create_mlp(int n_class){
    // define the memory
    ...
    model.layer3_w = ggml_new_tensor_2d(model.ctx, GGML_TYPE_F32, 64 , n_class);
    model.layer3_b = ggml_new_tensor_2d(model.ctx, GGML_TYPE_F32, n_class, 1);
...
int main(){
    ...
    // create the model
    int n_class = 10;
    struct mlp model = create_mlp(n_class);
    ...
    for (int epoch = 0; epoch < 10; epoch++){
        ...
        for (int step=0; step < mask.size(); step += batch_size){
            ...
            struct ggml_tensor * target = ggml_new_tensor_2d(
                ctx0, GGML_TYPE_F32, n_class, batch_size
            );
